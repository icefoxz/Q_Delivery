...
const loginForm = reactive<Login.ReqLoginForm>({
  username: import.meta.env.DEV ? "王时信" : "",
...
const login = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async valid => {
    ...
    try {
      ...
      const res: any = await $http.post("/api/Auth", {
        loginName: loginForm.username,
        loginPwd: md5(loginForm.password).toUpperCase()
      });
      ...
      ElNotification({
        title: getTimeState(),
        message: "欢迎登录 " + userStore.userInfo.user_Name,
        type: "success",
        duration: 3000
      });
    } finally {
