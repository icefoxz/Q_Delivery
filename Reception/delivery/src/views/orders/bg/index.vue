  ...
  <div class="swiper-container">
    ...
    <el-card shadow="hover" style="margin-top: 8px">
      ...
      >
        ...
        <el-table-column prop="tag_TypeName" label="标签类型" />
        <!-- <el-table-column prop="expand_Desc" label="标签描述" /> -->
        <el-table-column prop="order_TagOrReport_Name" label="说明" />
...
const handleQuery = async () => {
  loading.value = true;
  var res = await $http.get("/api/Order/getOrderTagOrReportList", queryParams.value);
...
const delSwiper = (row: any) => {
  ElMessageBox.confirm(`确定要删除吗?`, "提示", {
    ...
    .then(async () => {
      await $http.delete("/api/Order/deleteOrderTagOrReport", { id: row.id }, { params: { id: row.id }, body: { id: row.id } });
      handleQuery();
