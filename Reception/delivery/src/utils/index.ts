...
// 格式化时间函数
/**
 * @param {*} time 此参数要为时间对象
 * @param {*} onlyDate 是否只返回日期，可传入onlyDate
 * @param {*} needSeconds 如果需要返回的时间带秒数，可传入needSeconds
 * @param {*} separator 年月日之间的分隔符，默认为**年月日**
 * 格式化后的时间格式为xxxx年MM月xx日 xx:xx:xx
 */
export function formatTime(time: any, needSeconds: any, separator: any, onlyDate: any) {
  time = new Date(time);
